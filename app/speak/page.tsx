...
export default function SpeakPage() {
  ...
  const loadCardsForReview = () => {
    ...
    cardsToReview.forEach(card => {
      console.log(`Card: ${card.chinese}, Category: ${card.categoryId || 'none'}, Reading Level: ${card.readingReviewLevel}`);
    });
    ...
    if (successful) {
      ...
      setCards(prevCards => {
        ...
        if (currentCardIndex >= updatedCards.length && updatedCards.length > 0) {
          // We've removed the last card, adjust the index to the new last card
          setCurrentCardIndex(updatedCards.length - 1);
        } else if (updatedCards.length === 0) {
          // No more cards, set finished
          setIsFinished(true);
        }
      ...
      // If marked as "Again", move the current card to the back of the queue
      setCards(prevCards => {
        if (prevCards.length <= 1) {
          // If there's only one card, no need to move it
          return prevCards;
        }
        ...
        console.log(`Card moved to the back of the queue. Queue size: ${updatedCards.length}`);
        
        return updatedCards;
      });
      
      // If we're at the last card and moved it to the back, we need to reset to the first card
      if (currentCardIndex >= cards.length - 1) {
        setCurrentCardIndex(0);
      }
      // Otherwise, the current index now points to the next card since we removed the current one
    }
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  {speechSupported && (
                    <div className="mt-4 flex justify-center">
                      <AudioButton text={currentCard.chinese} size="md" />
                    </div>
                  )}
                </div>
              )}
